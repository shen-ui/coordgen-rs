...
fn atomic_idx_errors() {
    assert_eq!(
        ...
        Err(Error::AtomIdx { provided: 10, max: 1, atom: 0, bond_idx: 0 })
    );
    assert_eq!(
        gen_coords(&[1u8], &[[0u16, 10, 1]]),
        Err(Error::AtomIdx { provided: 10, max: 1, atom: 1, bond_idx: 0 })
    );
    assert_eq!(
        gen_coords(&[1u8], &[[1u16, 0, 1]]),
        Err(Error::AtomIdx { provided: 1, max: 1, atom: 0, bond_idx: 0 })
    );
    assert_eq!(
        gen_coords(&[1u8], &[[0u16, 1, 1]]),
        Err(Error::AtomIdx { provided: 1, max: 1, atom: 1, bond_idx: 0 })
    );

    assert_eq!(
        gen_coords(&[1u8, 2], &[[1, 0, 1], [3, 0, 1]]),
        Err(Error::AtomIdx { provided: 3, max: 2, atom: 0, bond_idx: 1 })
    );
    assert_eq!(
        gen_coords(&[1u8, 2], &[[1, 0, 1], [0, 3, 1]]),
        Err(Error::AtomIdx { provided: 3, max: 2, atom: 1, bond_idx: 1 })
    );
    // make sure error from first invalid one found
    assert_eq!(
        gen_coords(&[1u8, 2], &[[1, 3, 1], [0, 3, 1]]),
        Err(Error::AtomIdx { provided: 3, max: 2, atom: 1, bond_idx: 0 })
    );
}
...
fn bond_mult_errors() {
    assert_eq!(
        ...
        Err(Error::BondMult { provided: 0, bond_idx: 0 })
    );
    assert_eq!(
        gen_coords(&[1u8, 1, 2], &[[0, 1, 1], [1, 2, 0]]),
        Err(Error::BondMult { provided: 0, bond_idx: 1 })
    );
    // make sure error from first invalid bond found
    assert_eq!(
        gen_coords(&[1u8, 1, 2], &[[0, 1, 4], [1, 2, 0]]),
        Err(Error::BondMult { provided: 4, bond_idx: 0 })
    );
}
...
fn parallel_bond_errors() {
    ...
    assert_eq!(
        gen_coords(&[1u8, 1, 1, 1], &[[0, 1, 1], [1, 2, 2], [2, 3, 1], [1, 0, 2]]),
        Err(Error::ParallelBonds { bond1: 0, bond2: 3 })
    );
    assert_eq!(
        gen_coords(&[1u8, 1, 1, 1], &[[0, 1, 1], [1, 2, 2], [2, 3, 1], [2, 1, 2]]),
...
fn error_order() {
    ...
    assert_eq!(
        gen_coords(&atoms, &bonds),
        Err(Error::AtomIdx { provided: 100, atom: 0, bond_idx: 2, max: 4})
    );
    // if we fix it then second atomic index one errors
    bonds[2][0] = 2;
    assert_eq!(
        gen_coords(&atoms, &bonds),
        Err(Error::AtomIdx { provided: 150, atom: 1, bond_idx: 2, max: 4})
    );
    // if we fix it then bond multiplicity fails
    bonds[2][1] = 3;
    assert_eq!(
        gen_coords(&atoms, &bonds),
        Err(Error::BondMult { provided: 500, bond_idx: 2 })
    );
...
fn unchecked_panics_atomic_num() {
    unsafe { gen_coords_unchecked(&[0, 1], &[[0, 1, 1]]); }
}
...
fn unchecked_panics_atomic_idx() {
    unsafe { gen_coords_unchecked(&[1u8], &[[10u16, 0, 1]]); }
}
...
fn unchecked_panics_bond_mult() {
    unsafe { gen_coords_unchecked(&[1u8, 1], &[[1, 0, 0]]); }
}
...
fn unchecked_panics_parallel_bond() {
    unsafe { gen_coords_unchecked(&[1u8, 1], &[[1, 0, 1], [1, 0, 1]]); }
}
